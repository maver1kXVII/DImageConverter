...
namespace DImgConv.Compression
{
    ...
    public class LZW
    {
        ...
        static byte[] Decompress(BinaryReader input)
        {
            List<string> table = new List<string>();
            byte[] output1;

            FileStream outputStream = new FileStream("Decompressed.txt", FileMode.Create);
            StreamWriter output = new StreamWriter(outputStream, Encoding.ASCII);
            ...
            FileStream inputStream = new FileStream("Decompressed.txt", FileMode.Open);
            BinaryReader inputReader = new BinaryReader(inputStream);
            output1 = inputReader.ReadBytes((int)inputReader.BaseStream.Length);
            inputReader.Close();
            inputStream.Close();
        ...
        static void Decompress(BinaryReader input, StreamWriter output)
        {
            ...
            //while (input.PeekChar() != -1)
            //while (input.BaseStream.CanRead)
            while ((input.BaseStream.Length - input.BaseStream.Position) >= NumBytesPerCode)
            {
                int nextCode = ReadCode(input);
        ...
        public static void TestDecompres()
        {
